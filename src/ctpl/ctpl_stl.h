...
class thread_pool {
 ...
 private:
  ...
  std::vector<std::shared_ptr<std::atomic<bool>>> flags;  // false:thread stop ; true: thread run
  detail::Queue<std::function<void(int id)>*> q;
  std::atomic<bool> isDone;    // true: done; false: not done
  std::atomic<bool> isStop;    // true: stop; false: not stop
